% [mins,value] = LocalMinima(x, NotCloserThan, LessThan, MaxNumberOfResults)
%
...
%
% 4th optional argument let you determine how many indicies will be
% returned. 
%
% Integers will return a set of results with a
% total count equal to the integers magnitude. The sign of the
% integer determines which side of the distribution of minimum
% values the resultant set will contain. 
% 
% Decimals will return the specified percentile of the total set
% with the sign controlling which tail is being returned
%
% This program is the curse of my life.  Why can't things be simple?

function [Mins,Values] = LocalMinima(x, NotCloserThan, varargin)

[LessThan,MaxNumberOfResults] = DefaultArgs(varargin,{[],[]});
...
if ~isempty(MaxNumberOfResults)&MaxNumberOfResults~=0,
    if isempty(Mins),
        Values = nan(abs(MaxNumberOfResults),1);
        Mins   = nan(abs(MaxNumberOfResults),1);
    else
        if MaxNumberOfResults>0,
            [tval,ind] = sort(x(Mins));
        else
            [tval,ind] = sort(x(Mins),'descend');
        end
        NumberOfMins =  numel(tval);
        if NumberOfMins<MaxNumberOfResults,
            tMins = Mins;
            Values = nan(abs(MaxNumberOfResults),1);
            Mins   = nan(abs(MaxNumberOfResults),1);
            Values(1:NumberOfMins) = tval;          
            Mins(1:NumberOfMins)   = tMins(ind);
        else
            Values = tval(1:MaxNumberOfResults);
            Mins   = Mins(ind(1:MaxNumberOfResults));
        end
    end
else
    Values = x(Mins);
