...
% returned by matlab e.g. CCGHeart_7_1.mexext

dotpos = findstr(matver,'.');
verstr = matver(1:dotpos(2)-1);
verstr(findstr(verstr,'.'))='_';
%CCGFun = ['CCGHeart_' verstr '.' mexext];
CCGFun = ['CCGHeart_' verstr];
%CCGFun = 'CCGHeart_75';
%%
if ~exist(CCGFun,'file')
    CCGHeartPath = which('CCGHeart.c');
    CCGHeartPath = CCGHeartPath(1:end-10);
    fprintf('CCGHeart mex file for this system/version not found. \n Building one with name %s.%s for version=%s \n',...
        CCGFun,mexext, matver);
    %cmdstr = ['mex -output ' CCGHeartPath 'CCG' ' ' CCGHeartPath 'CCGHeart.c']; 
    cmdstr = ['mex -output ' CCGHeartPath ' ' CCGHeartPath 'CCGHeart.c'];
    eval(cmdstr);
end
