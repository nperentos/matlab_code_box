...
/*		mexPrintf("Allocating pair memory\n"); */
		...
		if (!Pairs) mexErrMsgIdAndTxt("xxx","Could not allocate memory for pairs");
	}
...
/*		mexPrintf("Reallocating pair memory ... ");
		...
		if (!pui) {
			...
			mexErrMsgIdAndTxt("xxx","Could not reallocate memory for pairs");
		}
		Pairs = pui;
*/
       mexPrintf("\n Number of pairs %d\n",PairCnt);
		mexErrMsgIdAndTxt("xxx","Too many pairs");
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
	...
	if (mxGetClassID(prhs[0])!=mxDOUBLE_CLASS || mxGetClassID(prhs[1])!=mxUINT32_CLASS
		...
		mexErrMsgIdAndTxt("xxx","Arguments are wrong type\n");
	}
	...
	if (mxGetNumberOfElements(prhs[1])!=nSpikes) mexErrMsgIdAndTxt("xxx","Number of marks ~= number of spikes");
	...
	for(i=0; i<nSpikes; i++) {
		...
		if (Mark==0) {
			mexErrMsgIdAndTxt("xxx","CCGEngine: No zeros allowed in Marks");
			abort();
		}
	}
	...
	if (!Times || !Marks || !Count) {
		mexErrMsgIdAndTxt("xxx","CCGEngine could not allocate memory!\n");
	}
