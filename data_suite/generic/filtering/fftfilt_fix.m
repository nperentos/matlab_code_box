% This is a fix provided by Mathworks, because the original version of
% fftfilt cannot handle long filters (to be applied to longer signals).
% However, this file wouldn't work, so I had to comment out the lines : 
% try
...
% If you need to use it, rename your old fftfilt.m ( found in Matlab/toolboxes/signal/signal) 
% to fftfilt_old.m and copy this one, renamed as fftfilt.m
...
%   $Revision: 1.8.4.5.4.1.2.1 $  $Date: 2010/06/15 19:04:59 $
...
if min(size(b))>1
   if (size(b,2)~=size(x,2))&&(size(x,2)>1)
      error(generatemsgid('InvalidDimensions'),'Filter matrix B must have same number of columns as X.')
   end
else
