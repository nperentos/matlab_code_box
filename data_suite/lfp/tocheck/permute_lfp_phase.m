% This function permutes (i.e. randomizes) the phase of the lfp signal
% while preserving the power (and the phase distribution).

function lfp = permute_lfp_phase(lfp)
orig_dim = size(lfp);
if orig_dim(2)>orig_dim(1); lfp=transpose(lfp); end;

lfp = fft(lfp); 

lfp=ifft(abs(lfp) .* exp(1i*angle(lfp(randperm(length(lfp))))),'symmetric'); % I NEED TO CHECK THIS ONE
