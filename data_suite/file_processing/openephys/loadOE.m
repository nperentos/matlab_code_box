% File history:
% load_open_ephys_data --> load_open_ephys_data_faster (Youngcho Kim) --> loadOE_fast (NK)
% Modified by NK to return int16 instead of double (not multiplying by bitVolt.
% Further modified to not use a function inside a function for the large data chunk

function [data, info, timestamps] = loadOE(filename)
display('running loadOE.m');
...
switch filetype
    ...
    case '.continuous'
        ...
        blockSize = sum(blockBytes(2:end));
        numIdx = floor((filesize - NUM_HEADER_BYTES)/blockSize)-1; % NK: added the -1, because otherwise it returns 1024 zeros at the end of the data
        ...
        %data = freadoff(sum(blockBytes(1:4)), fid, numIdx*SAMPLES_PER_RECORD, '1024*int16=>int16', blockSize-2*SAMPLES_PER_RECORD, 'b'); % read in data        
        fseek(fid, sum(blockBytes(1:4)), 'bof'); % replaced the above with these two lines % NK
        data = fread(fid, numIdx*SAMPLES_PER_RECORD, '1024*int16=>int16', blockSize-2*SAMPLES_PER_RECORD, 'b'); %NK
