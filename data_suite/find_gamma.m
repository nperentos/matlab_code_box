function ripples = find_gamma(x, varargin)

%% Input parsing
options = {'sr',1000,'method','hilbert','plot',1,'ripplefreq',[50 90],'mode','auto','thr',2.5,'mincycles',4,'refsig',[], 'periods',[],'wavelet_freq',0};
...
if strcmp(options.method,'hilbert')
    ...
    idx = (ripple_cycles<options.mincycles);
    ripple_dur(idx)=[];
    ripple_cycles(idx)=[];
    ripple_f(idx)=[];
    ripple_t(idx)=[];
