function [fig] = plotAmplitudePropagation(wf, varargin)
    ...
    % Channel overview
    %clf;
    fig = figure('Units','inches','Position',[0, 0, 16, 12]);
    subplot(5,5,[1, 2, 6, 7]);
    imagesc(squeeze(wf.waveFormsMean));
    xlabel('Time (ms)'); ylabel('Channel number'); 
    titelTemp = sprintf('Channel overview',id_unit);
    title(titelTemp);
    c1 = colormap(gca, colormap_BlueWhiteRed());  caxis([-1 1]*max(abs(caxis()))/2); box off;
    xlim([25 65]);
    ...
    for row = 1:size(tracesNorm, 1)
        ...
        if (min(tracesNorm(row,:))<-cutoffSig)
            plot(x_values, 1.5*tracesNorm(row, :) + row, 'LineWidth',2,'Color',[0 114/255 189/255]);
            % Find minimum
    ...
    plot(xlim,[0 0], '--', 'LineWidth', 1, 'Color', [0.8 0.8 0.8]); 
    plot([0 0],ylim, '--', 'LineWidth', 1, 'Color', [0.8 0.8 0.8]); 
    ...
    plot(xlim, [cutoffSig cutoffSig], '--', 'LineWidth', 1, 'Color', [0.8 0.8 0.8]);  
    box off;
    hold off;
