...
%                 to be placed
% (standard paths are defined if no input)
%
% Outputs:
% - No variables, just places the figures in path_output
%
% If something is wrong with 'xyzfeatures.mat', use
% ~/getAllFeatures/main_getAllFeatures.m to create the file new
%
...
addpath(genpath('/storage2/perentos/code/thirdParty/npy-matlab/'));
%savepath;

% Define paths if no input is defined
if nargin < 2 || (isempty(path_input) && isempty(path_output))
    path_output = '/storage2/perentos/data/SINAPS_2024__sorting/1950_2/';%'/storage3/paul/figures/';
    path_input = '/storage2/perentos/data/SINAPS_2024__sorting/1950_2/';%'/storage3/paul/data/1950_2/KS/';
end
...
% % fix shank spacing (NOT 500um but 250 um)
% ofs = 280;
...
load(fullfile(path_input,'channel_positions.mat'));
...
loadedData = load(fullfile(path_input,'xyzfeatures.mat'));
...
% Load Autocorrelograms
T = double(spike_times - spike_times(1));
G = double(spike_clusters);
BinSize = 1*round(double(0.5e-3*fs),0);
HalfBins = round(double(50e-3*fs),0)/10;
ccg = zeros(length(id_good),2*HalfBins +1);
t = zeros(length(id_good),2*HalfBins +1);
for i = 1:length(id_good)
    [ccg(i,:), t(i,:), ~] = CCG(T(G==id_good(i)),ones(sum(G==id_good(i)),1), BinSize,HalfBins,fs,[1],'hz');
end
acg = struct();
acg.y = ccg(:,HalfBins:end);
acg.x = t(:,HalfBins:end);


fprintf('Setup done, let''s load the clusters one by one (largest -> smallest) \n');

% Loop through all good clusters to get trajectories, positions & features
for j = 1:length(id_good)
    if ~isnan(featNorm(1,j)) && n_spikes(j) >= 300
        id_show = id_good(j);
        ...
        plotClusOnAtlas(wf,coordinates_reshaped, x_dendrite, y_dendrite, maxCoordinates);
        % Plot spatiotemporal propagation
        [fig] = plotAmplitudePropagation(wf);        % Add 2nd parameter cutoffSig and 3rd parameter ampCutOff
        ...
        plotFeatureSpace(featNorm, j,id_show, path_output,featureNames, fig);
...
% here plot the probe, the maxcoordinates and later superimpose the atlas
% image
...
% figure; plot(channel_pos_reshaped(:,1)-correction,channel_pos_reshaped(:,2),'r*')
