...
[singlepeak, singlepeakTime] = max(singleChan(troughTime:end));% after hyperpolarisation
...
singleFeature.PTratio = abs(singlepeak) / abs(trough); % peak to trough ratio 

yRepo = singleChan(troughTime:troughTime+2);
xRepo = [0, 1/fs, 2*1/fs];
dy = diff(yRepo);
dx = diff(xRepo);
singleFeature.repoSlope = mean(dy)/mean(dx); %slope of repolarisation
...
% on which electrode column is the max channel
...
% select channel with peak exceeding the cuttoff percentage
...
relAmplitudes = minAmpO(minAmpSigIdx) / minAmpTot; % amplitudes relative to 
