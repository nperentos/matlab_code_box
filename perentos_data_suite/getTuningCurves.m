...
% the attempt to make tun structure have 4 entries as opposed to one
% proved to be too messy. The other way to deal with the controlled
% exposure is to unfold the 4 trajectories (see below) into a single
% spatial map where the in- and out-bound are coded with distinct position
% codes eventhough they represent the same positions on carousel
...
% get contexts 
    ...
    % for each of the 4 contexts, lets pre-determine 
    % position extents
    % number of trials
    % occupancies

     for cx = 1:4
        myBlks{cx} = find(blks(:,4) == cx);
        myPeriods{cx} = blks(find(blks(:,4) == cx),[1:2]);    
        [myLoc{cx}, myT{cx}] = SelectPeriods(location,myPeriods{cx} ,[],[],0);
        % ensure correct position scale    
        locs2Keep{cx} = [min(myLoc{cx}):1:max(myLoc{cx})]';%locEdges = [min(tmpLoc):1:max(tmpLoc)]';
        locEdges = [1:1:720];
        %occupancy - should be rather flat (barring ramp in speed at start and end)    
        [occ]=histc(myLoc{cx},locEdges); %(unique(myLoc))
        occ = occ./sum(occ);
        occupancies{cx} = occ(locs2Keep{cx});
     end
     %occupancy1 = vertcat(occupancies{:}); % incompatible with canvas arrangement
     occupancy = [occupancies{2}; occupancies{1}; occupancies{3}; occupancies{4}];
    % trajectories need to be flip and shifted to make sense as follows:
    % cx = 1 (toL)    ~360-240deg desired is 0 to -120    ---> subtract max(cx1)
    % cx = 2 (fromL)  ~240-360deg desired is -120 to -240 ---> fliplr & subtract max(cx2) subtract min(c1_adj)
    % cx = 3 (toR)    ~360-480deg desired is 0 to 120     ---> subtract min(cx3)
    % cx = 4 (fromR)  ~480-360deg desired is 120 to 240   ---> fliplr & add max(cx3_adj)
    
    %predefine an empty canvas in which to inject the 4 subrasters        
    y = max(cellfun('length',myBlks));
    x = sum(cellfun('length',locs2Keep));    
    Ls = cellfun('length',locs2Keep)
    % 4 patches 
    ptch(1,:) = [Ls(2)+1,sum(Ls(1:2))          , 1, length(myBlks{1})];
    ptch(2,:) = [1, Ls(2)                      , 1, length(myBlks{2})];
    ptch(3,:) = [sum(Ls(1:2))+1, sum(Ls(1:3))  , 1, length(myBlks{3})];
    ptch(4,:) = [sum(Ls(1:3))+1, sum(Ls(1:4))  , 1, length(myBlks{4})];
    
% collect the four-part tuning curves into single canvases
    % trajectory to   left cage -  0    to -120 deg
    % trajectory from left cage - -120  to -240 deg
    % trajectory to   right cage - 0    to  120 deg
    % trajectory from right cage - 120  to  120 deg
    gU = sp.cids(sp.cgs == 1 | sp.cgs == 2);
    p = numSubplots(length(gU)); 
    canvas = nan(x,y,length(gU));
    
    for i = 1:length(gU) 
        i
        meanCellTuning_tmp = []; muFR_tmp = [];
        clear cellTuning_tmp;
        for cx = 1:4
            idxSp = find(sp.clu == gU(i)); 
            spT = round(sp.st(idxSp).*1000);
            res = zeros(size(tScale));% unsparsify
            res(spT) = 1; 
            mySpikes = res(myT{cx}); % keep spikes for movement periods            
            sz = [numel(session.helper.positionBinCenters)-1,1]; % offset by half a degree -360 to 360 matching location span

    %cell's mean tuning
            tmp1 = accumarray(myLoc{cx} , mySpikes , sz , @mean) .* (1/diff(tScale(1:2)));
            %meanCellTuning_tmp = [meanCellTuning_tmp; tmp1(locs2Keep{cx})];
            tmp1 = tmp1(locs2Keep{cx});
    % cell's firing rate    
            tmp2 = (sum(mySpikes)/length(mySpikes)) * (1/diff(tScale(1:2)));
            muFR_tmp = [muFR_tmp, tmp2];clear tmp2;        
    % cell's tuning split in trials
             
            cellTuning_tmp = zeros(sz(1), length(myBlks{cx}));%length(rewards.atStart));   
            for j = 1:length(myBlks{cx})
                from = myPeriods{cx}(j,1);
                to   = myPeriods{cx}(j,2);
                cellTuning_tmp(:,j) = accumarray(location(from:to),res(from:to),sz,@mean).*(1/diff(tScale(1:2))); % [numel(locEdges)-1,1]
            end
            cellTuning_tmp = cellTuning_tmp(locs2Keep{cx},:);
            if cx == 2; cellTuning_tmp = fliplr(cellTuning_tmp); tmp1 = fliplr(tmp1); end
            if cx == 4; cellTuning_tmp = fliplr(cellTuning_tmp); tmp1 = fliplr(tmp1); end
            X = ptch(cx,:);
            canvas(X(1):X(2),X(3):X(4),i) = cellTuning_tmp;                                
            meanCellTuning_tmp(ptch(cx,1):ptch(cx,2)) = tmp1;
        end
        meanCellTuning(i,:) = meanCellTuning_tmp;
        muFR(i) = mean(muFR_tmp);
    % Skaggs 1993 Information in bits per spike
        SSI(i) = nansum(occupancy.*meanCellTuning_tmp'/muFR(i).*log2(meanCellTuning_tmp'/muFR(i)));
    % trial similarities
        mu = mean(canvas(:,:,i),2);        
        w = 6; sd3 = 2;
        for j = 1:length(w)            
            for tr = 1:size(canvas,2)
                tmp = corrcoef(smooth_gauss(canvas(:,tr,i),w(j),sd3(j)),smooth_gauss(mu,w(j),sd3(j)),'rows','pairwise');    
                CC(tr,i) = tmp(2,1);
                cellTuningSm(:,tr,i) = smooth_gauss(canvas(:,tr,i),w(j),sd3(j));

            end
        end
        
    % covariance matrix of smoothed trials
        tmp = cov(zscore(cellTuningSm(:,:,i)));            
        tmp = tmp-diag(diag(tmp));   
        CV(:,:,i) = tmp;clear tmp;
    end  
    cellTuning = canvas; clear canvas;

    % going back to structure array for consistency with continuous, nothing more
    for i = 1:size(cellTuning,3)                
        tun(i).tuning(:,:) = cellTuning(:,:,i);
        tun(i).muTuning(:) = meanCellTuning(i,:);
        tun(i).muFR = muFR(i);
        tun(i).SSI = SSI(i);
        tun(i).cids = gU(i);
        tun(i).cellType = sp.cgs(sp.cids == gU(i));;
        tun(i).shank = nq.cluAnatGroup(find(nq.cids == gU(i))); %% this seems to be wrong but only for the first cell???
        tun(i).cluCh = nq.cluCh(find(nq.cids == gU(i))); %% this seems to be wrong but only for the first cell???
        tun(i).spkWidthR = nq.spkWidthR(find(nq.cids == gU(i))); %% this seems to be wrong but only for the first cell???
        tun(i).CC = CC(:,i)';
        tun(i).muCC = nanmean(CC(:,i));
        tun(i).tuningSm(:,:) = cellTuningSm(:,:,i);
        tun(i).CV = CV(:,:,i); 
    end
    %Xax = [locEdges(locs2Keep{2}), locEdges(locs2Keep{1}) locEdges(locs2Keep{3}) locEdges(locs2Keep{4})]-360;
    Xax = [-fliplr([1:sum(Ls(1:2))]), +[1:sum(Ls(3:4))]];
    lns=[-sum(Ls(1:2)), -Ls(1), -1, Ls(3), sum(Ls(3:4))];
    
%     figure; 
%     ha = tight_subplot(15,15,[0.01])
%     for i = 1:200%length(gU)
%         axes(ha(i));
%         pcolor(cellTuning(:,:,i)'); shading flat; set(gca,'ydir','reverse');
%         %hold on;
%         %plot([cumsum(Ls); cumsum(Ls)],[0 y+1],'linewidth',2,'color','r');
%         %xlabel('position (^o)');ylabel('trial #');
%         %pause;
%     end    
...
%% SAVE THE TUN STRUCTURE
    % resort cells according to cids and add some other vars
    [~,I] = sort([tun.cids]); 
    tun = tun(:,I);    
...
% raster of all trials
            ...
            elseif strcmp(session.info.taskType, 'controlled')
                ...
                % imagesc(tun(1).pos,[],tun(i).tuning');  figure
                pcolor(locDeg,1:1:size(tun(i).tuning,2),tun(i).tuning'); shading flat; set(gca,'ydir','reverse');
                clm = flipud(colormap('gray'));
                hold on;box off; set(gca,'TickDir','out');
                ylabel('trial number (#)'); xlabel('carousel position (^o)');
                colormap(clm);hold on;xlim([min(tun(1).pos)-1 max(tun(1).pos)+1]);%axis off;
                set(gca,'XColor','w');
                % add another axis with mean firing rate
                pos = get(gca,'pos');
                axes('pos',pos, 'YAxisLocation','right','color','none');
                lineClr = [1 0 0 0.5;0 1 0 0.5];
                plot(locDeg,smooth(mean(tun(i).tuning,2,'omitnan')),'linewidth',3,'color',lineClr(tun(i).cellType,:));                
                ylabel('firing rate (Hz)'); box off; set(gca,'TickDir','out');
                hold on; plot([tun1.lns(2:end-1); tun1.lns(2:end-1)],ylim,'r--');
                set(gca, 'YAxisLocation','right','color','none');
                xlim([min(tun(1).pos)-1 max(tun(1).pos)+1]);
                %hold on;
                %plot([cumsum(Ls); cumsum(Ls)],[0 y+1],'linewidth',2,'color','r');
                %xlabel('position (^o)');ylabel('trial #');
            end
...
% violin plot of the trial corcoef with the mean response            
                ...
                pnts = []; categ = [];
                pnts = [pnts, tun(i).CC(find(~isnan(tun(i).CC)))];
                categ = [categ,ones(length(tun(i).CC(find(~isnan(tun(i).CC)))),1)];    
                if ~isempty(pnts)
...
% in-field 1st nmf component activity across trials
            ...
            plot(gca,a,tmp(a),'or','markersize',6,'markerfacecolor','r');
...
% boxplot trials before versus after changepoint (in-field 1st nmf component activity)
            ...
            if tun(i).PFStability(1).chPnt_pval < 0.05
                plot(gca,1.5,max(ylim)/2,'*r','markersize',10,'LineWidth',2);
            end           
...
% histogram of the shuffle SSIs
            ...
            plot(gca,[tun(i).PFStability(1).trueSSI tun(i).PFStability(1).trueSSI],ylim,'--r','linewidth',2);
            hold on;
            if tun(i).PFStability(1).SSI_pval < 0.05
                plot(gca,tun(i).PFStability(1).trueSSI/2 ,max(ylim)/2,'*r','markersize',10,'LineWidth',2);
            end
...
function tt = joinTunNq(tun,nq)
    ...
    PFStability1_trueSSI                = nan(nTun);
    PFStability1_nullMap                = nan(nTun,size(tun(1).PFStability.nullMap,1),size(tun(1).PFStability.nullMap,2));
    PFStability1_SSI_pval               = nan(nTun);
    PFStability1_shuffleCorrectedSSI    = nan(nTun);
...
% PUT INTO A NEW TUN STRUCTURE
    tt.note                                   = {'cluster x pos x trial'; 'cellType: 1 for MUA, 2 for SU'; 'CC: corr coef b/w trial  and mean of all trials'; 'CV: trial-by-trial covariance matrix'; 'occupancy: if controlled, arranged as fromL, toL, toR, from R'};
...
%% raster in case oftun(1,4) structure type which is no more
