function MakeEvt(Input,OutFileName,Labels,SampleRate,Overwrite)

% make event file for neuroscopefunction MakeEvtFile(Input,varargin)
%function MakeEvtFile(Input,OutFileName,Labels,SampleRate,Overwrite)
%Input - one column or two column vector of times (lfp samples)
% Labels - string, cell array of strings or vector of label indexes (e.g.
% clu file)
% Overwrite  - 1, if 0 - append. 
%Output: in ms

%[OutFileName,Labels,SampleRate,Overwrite] = DefaultArgs(varargin,{[],[],1000,1});
