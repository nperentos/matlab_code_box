function location = getLocationTwoDirections(ch,rewards)
...
if nargin <2
    ...
    if isvector(ch)
        ...
        for i = 1:length(rewards.atStart)%soundIdx
            ...
            prev = find(exp(:,1) ==  rewards.atStart(i)) -1;
            if exp(prev,2) == 3 
                countDir = -1; 
            elseif exp(prev,2) == 1
                countDir = +1;
            else
                error('missing pulse aborting')
            end
            if countDir == -1
                if i == 1
            ...
            else % countDir == 1 that is, from 4200 or so to 0
                if i == 1
                    fill = find(location(1:tmp2) == 1);
                    for j = length(fill):-1:1
                        location(fill(j)) = length(fill)-j;
                    end
                end
                if i>=2
                    tmp1 = 30*rewards.atStart(i-1);%soundIdx
