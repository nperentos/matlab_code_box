function nq = getClusterPropertiesSINAPS(fileBase, varargin)
% getClusterPropertiesSINAPS.m generates a table that holds various cluster
% characterstics that can be used to filter which putative cells or MUAs
% are included in further analysis
% this version is specific for SINAPS data just for filestructure reasons

    options = {'pth',[],'verbose',1};
...
% load good clusters only
    ...
    Par.SampleRate = sp.sample_rate;
    SpkSamples = size(sp.temps,2);
    Channels = sp.n_channels_dat;
    Display = 1;
    Batch = 0;
    Overwrite = 0;
...
% cross and autocorelograms for all clusters
    display('computing all cross correlograms...'); tic
    ...
    save(['CCG.mat'],'ccg','tbin'); toc
    ACG = MatDiag(ccg);
    

% ACGs for burstiness calculation as ratio of spikes in <10 vs asymptote
    % (200-300ms)
    tic
    [ccgB tbinB ] = CCG(Res,Clu+1, round(1.5*Par.SampleRate/1000), 200, Par.SampleRate,uClu+1,'hz'); %200 is the number of bins on one side of the plot
    % save(['CCGB.mat'],'ccgB','tbinB'); ?2GB for sinaps
    ACGB = MatDiag(ccgB);    
    toc
    burstiness = mean(ACGB(202:210,:),1)./mean(ACGB(334:401,:),1); % 1.5-13.5ms and 200-300ms ranges
    save('burstiness.mat','ACGB','tbinB');
% 
...
%     end
    ...
    % get variable with channel groups and bad channels - prob needed for
    % GetChannelWaveform.m
    
% extract cluster average waveforms and assign peak channel
    display('assigning dominant channel to each cluster...');
    [avSpk, sdSpk,cluCh, cluShank, cluSpatialProps] = GetChannelWaveformSINAPS(fileBase);%, Chm
    % cluAnatGroup = tmp.kcoords(cluCh);
    anat_group = mapNP.anat_group(:);
    cluAnatGroup = anat_group(cluCh);
    %test = cell2mat(avSpk);
...
%     end
    ...
    % isiViolations
    [isiV, cluIDs] =  isiViolations(pwd);
    % find cluIDs that intersect with uClu
    cluIDs=(cluIDs') - 1; % begin at 0 like uClu
    for v = 1:length(uClu)
        refr(v) = isiV(find(cluIDs == uClu(v)));
    end    
    nq.refrac = refr;
    ...
    if Display
        ...
        for i = 2%1:2 % MUA and SUA
            ...
            for j = 1:length(cells)
                ...
                shadedErrorBar(([1:nq.SpkSamples*nq.ResCoef]-shift)*nq.Sample2Msec,ml*wv_tmp,wv_tmpSD, 'lineProps', cl); % (1:520)
                axis tight
                hold on
