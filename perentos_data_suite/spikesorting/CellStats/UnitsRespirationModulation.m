...
function OutArgs = UnitsRespirationModulation(FileBase,varargin)
%
[fMode, State, RefCh ] = DefaultArgs(varargin,{'compute','SWS', 'Respiration_Channel'});
FileBase = cdir;
...
switch fMode
    case 'compute'
        ...
        if isempty(Periods)
            out = [];
            fprintf('No valid periods')
            return;
        end
        
        load([FileBase '.RespirationParams.mat']);
        ...
        for i = 1:length(uClu)
            ...
            rt = RayleighTest(ResPh(myResLfp), ones(length(myResLfp),1));
            
%            ppc = PPC(ResPh(myResLfp));
            
            
            if isstruct(rt)
                
                out(j).FileBase = quality(j).FileBase;
                out(j).Layer = quality(j).Layer;
                out(j).cids = quality(j).cids;
                [out(j).hist.cnt out(j).hist.bins] = hist([ResPh(myResLfp); ResPh(myResLfp)+2*pi],36);
...
%                out(j).ppc = ppc;
        ...
        %             [out(i).hist.cnt out(i).hist.bins] = hist2([ResPh(ResLfp(In)), Clu(In); ResPh(ResLfp(In))+2*pi, Clu(In)],36,nClu);
