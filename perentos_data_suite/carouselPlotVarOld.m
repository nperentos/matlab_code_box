function carouselPlotVar(fileBase,varName,withmanipulation)
...
% INPUTS:
% fileBase
% varName: a subset of the following in a cell array
...
% first check that peripheralsPP.mat and varName exist
...
if sum(~cellfun(@isempty,regexp(pppNames,strjoin({varName},'|'))))>0 % request variable likely exists
    try
...
vars = ~cellfun(@isempty,regexp(pppNames,strjoin({varName},'|')));
vars = find(vars);
...
%% trial block averages (usually n of 30 or 40)
    ...
    text(0.5,0.95,['position resolved ',varName],'horizontalalignment','center','fontsize',20);xlim([0 1]);ylim([0 1]);axis off;
    ...
    print(fullfile(getfullpath(fileBase),[varName,'_plt']),'-djpeg','-r300');
end
