...
V_alpha = ones(1,length(Y));% NP edit 05/02/2024
F = [1 2 3 4];
...
for i=1:numel(X)-1
    ...
    A = [.5 .5 .5 .5]'; % NP edit 05/02/2024 i.e. fixed to constant alpha
    patch('Faces',F,'Vertices',V,'FaceColor',color_value,...
        'EdgeColor','none',...
        'FaceVertexAlphaData',A,'FaceAlpha','interp','AlphaDataMapping','none');%interp
end

plot(X,Y,'LineWidth',line_width,'Color','k');%color_value
hold off
