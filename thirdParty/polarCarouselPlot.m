function [varargout] = polarCarouselPlot(R,theta,Z,varargin)
...
p.addOptional('spokesPos',[]);
p.addOptional('direction','continuous');
p.parse(varargin{:});
...
if ~isempty(ncolor)
    ...
    %clm = [0.8 0.8 0.8; feval(myColorMap,100)];
    clm = [1 1 1; feval(myColorMap,100)];
    colormap(clm);
end
...
if ~ishold(cax);
    ...
    createSpokes(thetaMin,thetaMax,Ncircles,circPos,Nspokes,spokesPos);
    createCircles      (rMin,rMax,thetaMin,thetaMax,Ncircles,circPos  ,Nspokes);
    if strcmp(direction,'continuous')
        createRotationArrow(rMin,rMax,0,90,1,rMin - 5,1,direction);
    elseif strcmp(direction,'inbound')
        createRotationArrow(rMin,rMax,110,90,1,rMin - 5,1,direction);
        createRotationArrow(rMin,rMax,250,270,1,rMin - 5,1,direction);
    elseif strcmp(direction,'outbound')
        createRotationArrow(rMin,rMax,70,90,1,rMin - 5,1,direction);
        createRotationArrow(rMin,rMax,290,270,1,rMin - 5,1,direction);
    end
end
...
% Nested functions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function createSpokes(thetaMin,thetaMax,Ncircles,circlesPos,Nspokes,spokesPos)
        
        spokeMesh = linspace(thetaMin,thetaMax,Nspokes);
        if ~isempty(spokesPos)
            spokeMesh = spokesPos;
            Nspokes = length(spokeMesh);
            if max(spokeMesh)>360 | min(spokeMesh)>0
                error('spokesPos must be between 0-360');
            end
        end
        ...
        contourD = [contourD*0.95, contourD*1.05]; % np added this to extent the spokes for visibility
        ...
        for kk = 1:Nspokes
            ...
            plot(X,Y,'color',[0.4 0.4 0.4],'linewidth',1,...
...
%                 'handlevisibility','off');
            ...
            if and(thetaMin==0,thetaMax == 360),
                ...
                text(0.8.*contourD(1).*cost(kk),...
                    0.8.*contourD(1).*sint(kk),...
                    [num2str(spokeMesh(kk),3),char(176)],...
                    'horiz', 'center', 'vert', 'middle');
                %                 text(1.05.*contourD(end).*cost(kk),...
                %                     1.05.*contourD(end).*sint(kk),...
...
%             position = 1.*(spokeMesh(min(Nspokes,round(Ncircles/2)))+...
%                 spokeMesh(min(Nspokes,1+round(Ncircles/2))));            
            ...
            if abs(round(position)) ==90,
               ...
               text((contourD(kk)).*cosd(0-position),...
                    (mlt).*(contourD(kk)).*sind(0-position),...
                    rtick,'verticalalignment','BaseLine',...
                    'horizontalAlignment', 'right',...
                    'handlevisibility','off','parent',cax,...
                    'fontsize',10);
                ...
                % annotate spokes
%                 text(contourD(end).*0.6.*cosd(45-position),...
%                     contourD(end).*0.6.*sind(45-position),...
%                     [labelR],'verticalalignment','bottom',...
%                     'horizontalAlignment', 'right',...
%                     'handlevisibility','off','parent',cax);
            end
...
% np addition for direction of rotation arrows
    function createRotationArrow(rMin,rMax,thetaMin,thetaMax,Ncircles,circlePos,Nspokes,dr)        
        contourD = (circlePos/max(R))*max(xlim); 
        if isempty(circlePos)
            if strcmpi(Rscale,'linear')||strcmpi(Rscale,'lin'),
                tickMesh = linspace(rMin,rMax,Ncircles);
            elseif strcmpi(Rscale,'log')||strcmpi(Rscale,'logarithmic'),
        ...
        for kk=1:length(contourD)
            X = xGrid*contourD(kk);
            Y = yGrid*contourD(kk);
            plot(X(2:end-1),Y(2:end-1),'color','k','linewidth',2);
            if strcmp(dr,'outbound')
                plot(X(end),Y(end),'^','color','k','linewidth',2,'markerfacecolor','k');
            elseif strcmp(dr,'inbound')
                plot(X(end),Y(end),'v','color','k','linewidth',2,'markerfacecolor','k');
            elseif strcmp(dr,'continuous')
                plot(X(end),Y(end),'^','color','k','linewidth',2,'markerfacecolor','k');                
            end
        end
    end
